...
def parse(paths, output_directory, size, meta, label, channels=None, stitch=False, montage_size=30):
    """
    ...
    if ext == ".cif":
        for path in [paths]:
            _parse_cif(path, output_directory, size, meta, label, channels, stitch, montage_size)

    if ext in (".tif", ".tiff"):
        _parse_tif(paths, output_directory, size, meta, label, channels, stitch, montage_size)
...
def _parse_cif(path, output_directory, size, meta, label, channels, stitch, montage_size):
    ...
    if stitch:
        ...
        for channel in channels:
            for chunk in range(n_chunks):
                ...
                skimage.io.imsave(os.path.join(output_directory, "{}_{}_ch{:d}_{:d}.tif".format(meta, label, channel + 1, chunk + 1)), stitched_image) 
...
def _parse_tif(paths, output_directory, size, meta, label, channels, stitch, montage_size):
    ...
    if stitch:
        ...
        for i,channel in enumerate(channels):
            for chunk in range(n_chunks):
                ...
                images = [
                    _resize(skimage.io.imread(group_paths[i]), size) for group_paths[i] in list(groups.values())[chunk*chunk_size:(chunk+1)*chunk_size ]
                ]
        ...
        for group, group_paths in groups.items():
            ...
            if "smoothdisc" in os.path.basename(path).lower(): 
                classname = "SmoothDisc"                  
            if "crenateddisc_" in os.path.basename(path).lower():
                classname = "CrenatedDisc"
            if "crenateddiscoid" in os.path.basename(path).lower(): 
                classname = "CrenatedDiscoid"
            if "crenatedspheroid" in os.path.basename(path).lower(): 
                classname = "CrenatedSpheroid"                
            if "crenatedsphere" in os.path.basename(path).lower(): 
                classname = "CrenatedSphere" 
            if "smoothsphere" in os.path.basename(path).lower(): 
                classname = "SmoothSphere"                
            if "side" in os.path.basename(path).lower(): 
                classname = "Side"   
                

            #The third phrase is always Bag
            match = re.search('.*_[A-Za-z0-9]*_(.*)_.*_.*_Ch.*', os.path.basename(path))
            if match is None:
                print('Freaking bad, inconsistent naming: path = "{0}"'.format(path))
            bagID = match.group(1)
            del(match)
                

            if 'joseph' in os.path.basename(path).lower(): # Only Joseph puts his name at second phrase
                match = re.search('.*_[A-Za-z0-9]*_.*_(.*)_.*_Ch.*', os.path.basename(path))
                annotator = 'Joseph'
            else:
                match = re.search('.*_([A-Za-z0-9]*)_.*_.*_.*_Ch.*', os.path.basename(path))
                annotator = re.search('.*_[A-Za-z0-9]*_.*_(.*)_.*_Ch.*', os.path.basename(path)).group(1)
                if 'Minh-' in annotator:
                    annotator = 'Minh'
            if match is None:
                print('Freaking bad, inconsistent naming: path = "{0}"'.format(path))                      

            day = str(match.group(1)).upper() 
            del(match)
            

            #The phrase before "Ch" area is always Site
            match = re.search('.*_[A-Za-z0-9]*_.*_.*_(.*)_Ch.*', os.path.basename(path))
            if match is None:
                print('Freaking bad, inconsistent naming: path = "{0}"'.format(path))            
            site = match.group(1)
            ...
            output_pathname = os.path.join(
                output_directory,
                "{:s}__{:s}.npy".format(
                    classname+'_'+day+'_'+bagID+'_'+annotator+'_'+site,
                    hashlib.md5(str(time.time()).encode("utf8")).hexdigest())
            )
