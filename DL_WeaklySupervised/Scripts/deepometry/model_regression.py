...
class Model(object):
    ...
    def __init__(self, shape, units, directory=None, name=None):
        ...
        last = self.model.layers[-1].output
        regression_layer = keras.layers.Dense(self.units, activation=None)(last)
        self.model = keras.models.Model(self.model.input, regression_layer)
    ...
    def compile(self, lr=0.0001):
        ...
        self.model.compile(
            ...
            loss="mean_absolute_error", # regression
            metrics=[
                #"accuracy",
                "mae",
            ],
    ...
    def fit(self, x, y, x_valid=None, y_valid=None, batch_size=32, class_weight="auto", balance_train=True, balance_valid=False, train_valid_sameset=True, validation_split=0.2, mixup_alpha=0.0, finetune=False, old_shape=None, old_class=None, old_model=None, trainable_layer='pool5', epochs=512, verbose=0):
        """
        ...
        options = {
            "callbacks": [
                ...
                keras.callbacks.TensorBoard(log_dir= pkg_resources.resource_filename("deepometry", os.path.join("./deepometry/data")), histogram_freq=0, batch_size=32, write_graph=True, write_grads=False, write_images=False)
            ],
        ...
        self.model.fit_generator(
            generator=train_generator.flow(
                ...
                # y=10*y_train/48.0 + numpy.random.normal(loc=0.0, scale=0.3, size=y_train.shape), # regression, normalize this, make this more continue
                y=10*(y_train/48.0) - 5.0,
                batch_size=batch_size,
            ...
            validation_data=valid_generator.flow(
                ...
                y=10*(y_valid/48.0) - 5.0, # regression,
                batch_size=batch_size,
    ...
    def _resource(self, filename):
        ...
        if self.directory is None:
            return pkg_resources.resource_filename(
                "deepometry",
                os.path.join("./deepometry/data", resource_filename)
            )
